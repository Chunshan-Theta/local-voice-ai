...
build_and_push() {
    ...
    echo "Building $name..."
    docker build -t "localhost:5000/$name:latest" -f "$dockerfile" "$context"
    echo "Pushing $name..."
...
build_and_push "local-voice-ai-breezyvoice" "BreezyVoice/Dockerfile" "BreezyVoice/" &
